"""Étude 013 : la question de l'étude 011 sur quarante ans de survivants et six caractéristiques de prix.

Le point d'entrée ne porte aucune logique réutilisable : il lit le panneau
quotidien de l'étude 002 dans la couche bronze du lac, en fait un panneau
mensuel de caractéristiques de prix, et rejoue le protocole de l'étude 011. Il
est déterministe et sort sur le réseau pour le seul taux sans risque.

    uv run python studies/013_cross_sectional_ml_long/run.py
"""
...
from quantlab.analytics.ic import ic_series, ic_summary, quantile_returns
...
def load_panel(config: dict[str, Any]) -> tuple[Panel, pd.DataFrame, dict[str, Any]]:
    """Assemble le panneau mensuel de caractéristiques de prix depuis la couche bronze du lac."""
    frame = read_table(
        str(config["data"]["dataset_id"]), "bronze", columns=["date", "symbol", "adj_close"], engine="pandas"
    )
    prices = frame.pivot(index="date", columns="symbol", values="adj_close").sort_index()  # noqa: PD010
    prices.index = pd.DatetimeIndex(prices.index)
    monthly = prices.resample("ME").last()
    returns = monthly.pct_change().loc[config["data"]["start"] : config["data"]["end"]]
    ...
    features = price_features(returns, None)
    features = features[features["rev_1"].notna()]
...
def main() -> None:
    ...
    with stage("portefeuilles"):
        ...
        universe_note = (
            f"{coverage['n_entities']} survivants du S&P 500 d'aujourd'hui, prix Yahoo depuis 1985, "
            "biais de survie déclaré, déciles long moins court"
        )
        for name, out in outputs.items():
    ...
    with stage("biais_de_survie"):
        # Les trois signaux de prix qui ont un équivalent chez Kenneth French, en
        # décile long moins court : sur nos survivants, et sur les déciles CRSP
        # sans biais de survie. L'écart entre les deux mesure ce que la survie
        # ajoute ou retire à chaque signal. Le renversement à long terme de
        # French porte sur 60 à 13 mois, le nôtre sur 36 à 13, écart déclaré.
        start, end = (
            pd.Timestamp(metrics["test_window"]["start"]),
            pd.Timestamp(metrics["test_window"]["end"]),
        )
        real_wide = predictions_to_wide(panel.label.rename("y"))
        french = FrenchProvider()
        control_rows = []
        for signal, dataset in (
            ("mom_12_1", "10_Portfolios_Prior_12_2"),
            ("rev_1", "10_Portfolios_Prior_1_0"),
            ("mom_36_13", "10_Portfolios_Prior_60_13"),
        ):
            sig_wide = predictions_to_wide(panel.features[signal].rename("s"))
            survivors = quantile_returns(sig_wide, real_wide, n_quantiles=int(pcfg["n_quantiles"]))["spread"]
            survivors = survivors.shift(1).dropna()
            tables = french.fetch_tables(dataset)
            # Le fichier des déciles de renversement à un mois titre son premier
            # bloc « Aerage Value Weighted Returns », coquille de la source
            # mesurée le 2026-09-03 ; le tableau se choisit donc par une règle.
            table_name = next(n for n in tables if "value_weight" in n and "monthly" in n)
            deciles = tables[table_name]
            kf = (deciles.iloc[:, -1] - deciles.iloc[:, 0]).dropna()
            kf.index = _month_end(kf.index)
            both = pd.concat([survivors.rename("survivants"), kf.rename("french")], axis=1, join="inner")
            both = both.loc[start:end].dropna()
            for name in both.columns:
                series = both[name]
                control_rows.append(
                    {
                        "signal": signal,
                        "dataset": dataset if name == "french" else "survivants",
                        "n_months": len(series),
                        "mean_annual_pct": float(series.mean() * 12 * 100),
                        "sharpe": float(sharpe_ratio(series, frequency=MONTHLY)),
                        "max_drawdown_pct": float(max_drawdown(series) * 100),
                        "correlation_survivants_french": float(both["survivants"].corr(both["french"])),
                    }
                )
        control = pd.DataFrame(control_rows)
        _write_table(control.set_index(["signal", "dataset"]), "survivorship_control")
        summary = {}
        for signal in control["signal"].unique():
            rows = control[control["signal"] == signal].set_index("dataset")
            summary[signal] = {
                "sharpe_survivants": float(rows.loc["survivants", "sharpe"]),
                "sharpe_french": float(rows.drop(index="survivants")["sharpe"].iloc[0]),
                "mean_annual_pct_survivants": float(rows.loc["survivants", "mean_annual_pct"]),
                "mean_annual_pct_french": float(rows.drop(index="survivants")["mean_annual_pct"].iloc[0]),
                "correlation": float(rows.loc["survivants", "correlation_survivants_french"]),
            }
        metrics["survivorship_control"] = summary
    ...
    with ExperimentRegistry().run(
        name="cross_sectional_ml_long_013",
        hypothesis=config["hypothesis"],
        config={k: v for k, v in config.items() if k != "verdict"},
        seed=seed,
        universe=[f"survivants S&P 500, {coverage['n_entities']} titres"],
