...
téléchargement. Le passage des prix aux tableaux mensuels est celui de l'étude,
appelé par son nom et non recopié. Le script écrit la série mensuelle brute,
les poids exécutés et le terme de financement qui sépare un rendement
excédentaire d'un rendement total. C'est cette série que LEAN doit retrouver.
"""
...
def main() -> None:
    ...
    resultat = run_backtest(
        weights=poids,
        returns=entrees.monthly_excess.loc[debut:].fillna(0.0),
        cost_model=None,
        execution_lag=1,
    ...
    taux_mensuel = rf_mensuel.reindex(executes.index, method="ffill")
    financement = (resultat.net_exposure * taux_mensuel).rename("financing")
    ...
    entrees.monthly_volatility.loc[debut:].to_csv(TABLES / "lab_monthly_volatility.csv", index_label="date")
    ...
    resume = {
        ...
        "gross_leverage_mean": float(resultat.gross_exposure.mean()),
        "gross_leverage_max": float(resultat.gross_exposure.max()),
    }
